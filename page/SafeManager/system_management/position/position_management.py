...
position_management_url = GetUrl().get_admin_url() + '#/systemManage/system/system-position-module.html'


class PositionManagement(ManagerLogin):
    """职务管理页面元素操作及返回值"""
    ...
    def no_input_name_add_position(self):
        """
        不输入职务名称，点击保存
        :param name: 新增职务的名称
        :return:错误提示字段
    ...
    def editor_position_click(self, name):
        """
        点击编辑按钮
        :return:
        """
...
if __name__ == '__main__':
    driver = browser()
    d = PositionManagement(driver)
    d.open_url(manager_url)
    d.yf_manager_login()
    time.sleep(2)
    d.open_url(position_management_url)
    d.editor_position('asc')
